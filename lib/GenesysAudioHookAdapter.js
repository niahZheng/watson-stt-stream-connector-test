...
function handleAudioHookConnection(webSocket, incomingMessage) {
  ...
  // 记录新的连接详情
  logger.info({
    event: 'ws_connection_received',
    remoteAddress: incomingMessage.socket.remoteAddress,
  ...
  webSocket.on('message', (data) => {
    if (typeof data === 'string') {
      try {
        ...
        logger.error('WebSocket message parsing failed', e);
        webSocket.close(1000, 'Invalid message format');
      }
    } else if (Buffer.isBuffer(data)) {
      // 每收到100个音频包记录一次统计信息
      if (sessionState && sessionState.receivedBufferCount % 100 === 0) {
        logger.debug({
          event: 'ws_audio_stats',
          sessionId: sessionState.sessionId,
